    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <Button
          variant="secondary"
          className={cn(
            "hidden h-11 min-w-36 justify-between px-3 text-muted lg:inline-flex",
            onDarkSurface &&
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content asChild aria-describedby="command-description">
          <m.div
            className="fixed left-1/2 top-[18vh] z-50 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 overflow-hidden rounded-xl border border-border bg-surface shadow-2xl"
            ...
            <Command label="Portfolio navigation">
              <div className="flex h-14 items-center gap-3 border-b border-border px-4">
                ...
                <Dialog.Close asChild>
                  <Button variant="ghost" className="size-10 p-0" aria-label="Close quick navigation">
              ...
              <Command.List className="max-h-80 overflow-y-auto p-2">
                ...
                <Command.Group heading="Sections" className="command-group">
                  {navigation.map((item) => {
                    const Icon = destinationIcons[item.label];
